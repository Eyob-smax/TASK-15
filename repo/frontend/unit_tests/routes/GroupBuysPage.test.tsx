...
vi.mock("@/lib/auth", async () => {
  ...
  return {
    ...actual,
    useAuth: () => mockUseAuth(),
    RequireRole: ({ children }: { children: React.ReactNode }) => (
      <>{children}</>
    ),
  };
});
...
vi.mock("@/lib/hooks/useCampaigns", () => ({
  ...
  useCreateCampaign: () => ({
    mutateAsync: mockCreateCampaignMutateAsync,
    isPending: false,
  }),
}));
...
describe("GroupBuysPage", () => {
  ...
  it("opens the member start flow from an item context and locks the item id", async () => {
    renderPage("member", [
      "/group-buys?item_id=11111111-1111-1111-1111-111111111111",
    ]);
  ...
  it("shows offline queue notice when creating while offline", async () => {
    mockUseOfflineStatus.mockReturnValue({
      isOnline: false,
      isOffline: true,
